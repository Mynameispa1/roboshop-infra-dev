resource "aws_cloudfront_distribution" "pavankumarmuvva" {
  enabled             = true
  ...
  records = [
    {
      ...
      type    = "A"
      alias   = {
        name    = aws_cloudfront_distribution.pavankumarmuvva.domain_name
        zone_id = aws_cloudfront_distribution.pavankumarmuvva.hosted_zone_id
      }
    }
  ]
}